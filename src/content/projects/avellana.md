...
- **iOS voice sim fix: AI audio over WS as base64 JSON text frames** -- iOS WebKit (Chrome iOS 147 / iOS 26.3.x and Safari / iOS 18.7) was RSTing the WSS connection within ~30-100 ms of receiving the first inbound binary frame. Desktop browsers worked fine on the same network, ruling out network/Railway/Cloudflare. Confirmed the bug class via three progressive isolation toggles in `SimRunScreen` (`?debug=noaudio`, `?debug=nomic`, `?debug=passive`); the `?debug=passive` run produced the smoking gun — only WS attempts that received zero binary frames lived 30+ s. Fix: `pump_gemini_to_ws` in `apps/api/app/routes/sessions.py` now sends AI audio as `{"event":"audio_b64","b64":"..."}` JSON text frames via `send_json` instead of `send_bytes`. Frontend `onmessage` decodes via `atob` into an `ArrayBuffer` and feeds the same `scheduleChunk` audio pipeline. ~33% bandwidth overhead — negligible at pilot scale. Mic upload (client → server) stays binary because only inbound binary frames trigger the iOS bug. Documented in [ADR-009](../adr/009-ai-audio-over-ws-text-frames.md). Side-fixes shipped in the same arc: `releaseMicStream()` on sim end so iOS drops the orange recording indicator; desktop browsers auto-fire `handleStart` on mount (the tap-to-start gate is now mobile-only); 4xxx WS closes route to "Start a new simulation" instead of looping the Try Again retry against an unrecoverable session
- **Restore-seed-characters super-admin endpoint + test self-cleanup** -- `POST /api/v1/admin/orgs/{org_id}/restore-seed-characters` flips ARCHIVED → PUBLISHED on every seed-tagged character in an org. Idempotent. Used by the property-bootstrap-seed-characters scenario test in its `finally` block to undo the destructive delete in step 5 — without this, the scenario was self-eroding the live seed library because `bootstrap_seed_characters` skips any seed_id that already exists in the org (including ARCHIVED rows). After ~23 weeks of test runs, the live library had eroded to 5 couples + 2 anonymous characters with zero solo non-anonymous left, breaking the test's `find` for a delete target. Endpoint also runs as a one-shot fix to restore eroded staging/dev DBs (local dev was at 7/30 live; one curl restored to 30/30). Three new auth-gate unit tests in `test_admin_routes.py` cover super-admin 200, director 403, unknown org 404
...
97. **iOS voice sim debug handoff** — multi-screen handoff describing 3-4 short audio bursts then silence on iOS Chrome 147 / iOS 26.3.1 + weak cellular, with a list of confirmed-NOT-the-issue layers (backend/Gemini healthy, audio pipeline running, mic streaming, tap-to-start UX, backend handle recovery on 4404) and ~5 suggested next moves
98. **First sim test, three short bursts** — "just did a sim with ramy haddad and heard 3 short bursts of audio, that's it. can you check?"
99. **Safari smoke + mic-still-on observation** — screenshot of "Connection lost (1006: network dropped)" plus "my mic seems to be still on after i stop the sim, based on the orange light of the iphone"
100. **Network/desktop sanity** — "1. wifi 2. on desktop this works well 3. don't think this is related to any of that. please ship the mic light. Also, i just saw the screen on desktop that i need to accept to hear the guest. that should not show up. can you please continue debugging?" — eliminated network/transport hypotheses, opened the desktop tap-to-start regression
101. **`?debug=noaudio` test result** — "i saw status: live and status:connecting switching back and forth a couple of times, then it stayed on status:live for a while" — first datapoint that ruled out the audio scheduling theory but stayed ambiguous
102. **`?debug=passive` test, the smoking gun** — "it bounced between live and connecting maybe 4-5 times. then it staued in live for over 30 seconds" — proved the bug only fires on inbound binary frames; with zero binary frames in, the WS lives indefinitely
103. **Try Again loop on COMPLETED sessions** — screenshots of "Connection lost (4404: session not found, try starting a new one)" on both Chrome iOS and Safari iOS, surfacing a UX bug where Try Again kept retrying against a session the user had already manually ended
104. **`?debug=textaudio` candidate fix verified** — "textaudio is working well. can you please ship the fix, test, document, etc?" — green-light to flip base64-text-frame audio from a debug toggle to the unconditional default for all clients
105. **Bug-class retrospective** — "while we wait, can you please summarize why this bug was so hard to catch in the first place, why it doesn't affect desktop, and whether there's a chance this still doesn't work on Android, a platform i haven't tested on yet?" — produced a written retrospective covering the false-positive correlation, the multiple-symptoms-from-one-root pattern, the desktop vs iOS WebKit codebase split, and the Android Chromium = real-Chromium baseline
106. **Phase B finalize handoff to follow-up agent** — "won't have android for a while. anything else you recommend doing now?" → "let's do A now" — chose to ship Fix A for the bootstrap test (test-side cleanup via a new restore-seed-characters super-admin endpoint) over Fix B (changing bootstrap product semantics) so the destructive scenario test could no longer permanently archive a seed character per run
107. **Triage + portfolio housekeeping** — "i already chedked it works. please run sentry triage, merge updates, and once everything looks good, update the portfolio, please" — ran `merge-updates` on a 2-day-pending history-audio-playback update file, ran `sentry-triage` on 12 unresolved issues (all downstream of the now-fixed iOS WSS bug, all resolved), then this portfolio refresh
108. **Sentry resolve confirmation** — "yes" — bulk-resolved all 12 unresolved Sentry issues via `node scripts/sentry.mjs resolve <SHORT-ID>` in a loop, leaving the inbox empty as the regression signal for the 2026-05-14 follow-up agent
109. **Schedule the follow-up + dig into the gate failure** — "yes, please. and let's look at the gate test that was failing" — created a one-shot remote agent for 2026-05-14 (verify iOS fix still working, search WebKit changelogs for the upstream binary-frame fix, audit the bootstrap test) and traced the gate failure to a destructive-test self-erosion pattern
...
> "just did a sim with ramy haddad and heard 3 short bursts of audio, that's it. can you check?"

> "i just tried safari. i heard some initial audio burst, then silence, then i heard the audio playing well for about 5-7 seconds. then i tried talking back to it, and got this:"

> "also, my mic seems to be still on after i stop the sim, based on the orange light of the iphone"

> "1. wifi 2. on desktop this works well 3. don't think this is related to any of that. please ship the mic light. Also, i just saw the screen on desktop that i need to accept to hear the guest. that should not show up. can you please continue debugging? please ultrathink if needed to get it right, and perhaps look into ways you can test some of the proposed fixes yourelf before asking me for human cycles to verify"

> "i saw status: live and status:connecting switching back and forth a couple of times, then it stayed on status:live for a while."

> "it bounced between live and connecting maybe 4-5 times. then it staued in live for over 30 seconds"

> "textaudio is working well. can you please ship the fix, test, document, etc?"

> "while we wait, can you please summarize why this bug was so hard to catch in the first place, why it doesn't affect desktop, and whether there's a chance this still doesn't work on Android, a platform i haven't tested on yet?"

> "won't have android for a while. anything else you recommend doing now?"

> "i already chedked it works. please run sentry triage, merge updates, and once everything looks good, update the portfolio, please"

> "yes, please. and let's look at the gate test that was failing"

> "let's do A now, please"
...
### iOS WebKit RSTs WSS connections after the first inbound binary frame

**Problem:** Live voice simulations died within ~30-100 ms on every iOS device tested (Chrome iOS 147 / iOS 26.3.x and Safari / iOS 18.7), regardless of whether the user was on WiFi or cellular. Desktop browsers worked fine on the same network. The pattern from Sentry: `audio_msgs_forwarded=18, exit_reason=ws_send_bytes_failed` from the backend pump, code 1006 abnormal closure on the frontend, ~71 ms gap between the first inbound chunk and the WS death every single time. Multiple early hypotheses misdirected the investigation: a 2-byte tiny silence chunk from Gemini that "looked like the trigger," a ~71 ms timing window that mapped beautifully onto the audio buffer schedule (chunk at +20 ms, plays for 40 ms, ends at +60 ms, WS dies at +71 ms — coincidence not causation), a Safari-specific `keepalive ping timeout` that turned out to be a downstream symptom of the same root, and a `4404 Try Again` loop that was a UX bug stacking on top of the WS bug.

**Root cause:** iOS WebKit (which powers iOS Safari and — via Apple's "all iOS browsers must use WKWebView" rule — Chrome iOS too) RSTs the WSS connection within ~30-100 ms of the first **inbound** binary WS frame. Asymmetric: outbound binary (mic upload) is fine. The bug fires regardless of frame size (we saw it on 2 B, 1920 B, 2400 B, 4800 B), regardless of frame content, and regardless of any frontend handling of that frame. Desktop browsers don't share this code path — Chromium has its own Net stack, Firefox has Gecko, Safari macOS uses a different WebKit build than iOS. So same WiFi works on the laptop but breaks on the phone.

**Solution:** Switched the AI audio stream from binary WS frames to base64-encoded JSON text frames over the same WSS. Backend `pump_gemini_to_ws` always serializes audio as `{"event":"audio_b64","b64":"..."}` via `send_json`. Frontend `onmessage` decodes via `atob` into an `ArrayBuffer` and feeds the same `scheduleChunk` audio pipeline the binary path used. Different WebKit code path, unaffected by the bug. ~33% bandwidth overhead — negligible at pilot scale, well worth the cross-platform compatibility. Mic upload (client → server) stays binary because only inbound binary frames trigger the bug. Documented in [ADR-009](../adr/009-ai-audio-over-ws-text-frames.md) with the alternatives considered (WebRTC DataChannel, HTTP/2 streaming) and the bandwidth trade-off accepted.

**Debugging approach:** Three progressive isolation toggles in `SimRunScreen.tsx`, each ruling out one hypothesis: `?debug=noaudio` (skip `scheduleChunk` — does the audio scheduling kill the WS?) → still dies at ~104 ms → no, audio scheduling is fine. `?debug=nomic` (skip mic `ws.send`) → still dies → no, outbound traffic is fine. `?debug=passive` (skip the mic processor wiring entirely so no audio data flows in either direction, only WS receive runs) → the decisive run. The only WS attempts that lived 30+ seconds were the ones in which Gemini happened to be idle and zero binary frames arrived. As soon as a single inbound binary frame landed, RST followed within ~100 ms. That isolation made the bug class unmistakable: "first inbound binary frame triggers iOS to RST, regardless of what the page does next." A `?debug=textaudio` toggle was shipped as the candidate fix, verified end-to-end by Fernando on iOS over a full bidirectional voice conversation, then flipped to the unconditional default. Backend now also captures `ws_send_exc_type` and `ws_send_exc_msg` on send failures (was only the exit reason before) so the next regression names its exception type instead of "send failed." Memory note `feedback_ios_wss_binary_frame_bug.md` codifies the bug class for future sessions.

### Property-bootstrap-seed-characters scenario was destructively self-eroding the seed library

**Problem:** During the iOS-fix Phase B finalize, the scenario gate failed on `property-bootstrap-seed-characters.spec.ts:125` with `expect(target).toBeTruthy()` getting `undefined`. The test was looking for a non-anonymous, non-paired seed character to delete in step 5, and `find` returned `undefined`. Direct query of the live seed library returned 7 characters: 5 couples (Tanaka, Novak, Méndez, Sharma, Chen-Williams) + 2 anonymous (External Caller, Internal Caller) = 0 solo non-anonymous. The seed library defines 30 characters; 23 were missing.

**Root cause:** The scenario was destructive and didn't self-restore. Step 5 deletes one solo non-anonymous seed per run. Step 6 re-runs `bootstrap_seed_characters`, which iterates `SEED_CHARACTERS` and skips any `seed_id` already present in the org — including ARCHIVED rows (since soft-archive sets `status = ContentStatus.ARCHIVED` but keeps the row, and `_existing_seed_ids` reads ALL characters regardless of status). So every test run permanently soft-archived one solo non-anonymous seed and the bootstrap considered the seed_id "covered." Over weeks of test runs, all 23 solo non-anonymous seeds in the dev org accumulated as ARCHIVED, leaving only 5 couples + 2 anonymous live. The next run couldn't find a target.

**Solution:** Fix A from a small menu (test-side cleanup, leaves product semantics unchanged): a new super-admin endpoint `POST /api/v1/admin/orgs/{org_id}/restore-seed-characters` flips ARCHIVED → PUBLISHED on every seed-tagged character in the org. Idempotent. Touches only rows where `status == ARCHIVED` and `tags` contains 'seed' — never disturbs PUBLISHED/DRAFT rows or non-seed characters. Audit-recorded. The scenario's `finally` block now calls this endpoint before the property cleanup, so whatever step 5 deleted (and any other archived seeds left over from prior runs) gets restored. Three new auth-gate unit tests cover super-admin 200, director 403, unknown org 404. The actual SQL semantics are integration-tested by the scenario itself. The alternative (Fix B — change `bootstrap_seed_characters` to also restore archived rows on re-run) was rejected because it would change product semantics: a director who deliberately archived a seed character would see it reappear after the next property bootstrap, which is wrong product behavior.

**Debugging approach:** Direct query against the live API (`/api/v1/orgs/{org_id}/content/characters?limit=200&include_archived=true`) revealed total=34, seed=30 with 7 live + 23 archived. The composition (5 couples + 2 anonymous, 0 solo non-anonymous live) matched the test's `find` predicate exactly — the test looks for a character that's neither a couple (no `&` in name) nor anonymous, and that subset was empty. Read `bootstrap_seed_characters` and `_existing_seed_ids` to confirm the skip-on-existing-seed-id logic includes archived rows. After shipping the new endpoint, ran it locally via curl with a super-admin JWT — restored 23 rows — re-ran the failing scenario locally — passed in 24.3 s. Verified the `finally` block restores even after a fresh delete by re-querying live count (back to 30/30 with 23 solo non-anon).
...
- **AI audio over WSS as base64 JSON text frames, not binary frames** -- `pump_gemini_to_ws` always serializes outbound AI audio as `{"event":"audio_b64","b64":"..."}` JSON text via `send_json`. The binary `send_bytes` path is gone. Reason: iOS WebKit (which powers Safari iOS and — via Apple's "all iOS browsers must use WKWebView" rule — Chrome iOS too) RSTs the WSS connection within ~30-100 ms of receiving the first inbound binary WS frame, regardless of frame size or content. Confirmed by progressive isolation toggles: `?debug=passive` sessions that received zero binary frames lived 30+ seconds; sessions that received any inbound binary frame died within ~100 ms. Text frames go through a different WebKit code path that isn't affected. ~33% bandwidth overhead on the AI audio stream, negligible at pilot scale. Mic upload (client → server) stays binary because only inbound binary frames trigger the iOS bug — the asymmetric breakage was itself a useful diagnostic signal. Documented in [ADR-009](../adr/009-ai-audio-over-ws-text-frames.md) with the alternatives considered (WebRTC DataChannel, HTTP/2 streaming) and a 2-week scheduled remote agent that will check Apple's release notes for the upstream WebKit fix so we can revert and reclaim the bandwidth once iOS ships a patch
- **Diagnostic toggles preserved, not deleted** -- the `?debug=noaudio`, `?debug=nomic`, and `?debug=passive` URL flags shipped during the iOS investigation are kept in production. They cost nothing in production (a `searchParams` read and a few branches) and saved hours of debugging time once we needed them. The principle: when a diagnostic toggle has paid for itself, leaving it in place insures against the next regression in the same code path
